...
def get_ai_message(user_message):
    ...
    다만 context에 관련 근거가 없으면 "제공된 자료에서 관련근거를 찾지 못했습니다."로 답해주세요.
    컨텍스트라는 단어를 사용해야 할 때는 컨텍스트(context)대신에 자료로 바꿔서 답해주세요.
    컨텍스트에 관련 근거가 없는 경우에도 "제공된 자료에서 관련근거를 찾지 못했습니다."하고 추가로 너의 의견을 최대한 합리적은 근거를 바탕으로 답해주세요. 
    
    context에 관련 근거가 있으면 아래의 형식으로 답변해주시고, 답변 시 예시를 참고해 주세요. 
    <형식>
    {regulation} ,\n 답변내용
    </형식>
